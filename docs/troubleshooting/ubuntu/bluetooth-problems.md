# Troubleshooting Bluetooth Problems on Ubuntu

This guide covers common steps to fix Bluetooth issues on Ubuntu, such as devices not connecting, not being detected, or adapters not being found.

---

## Step 1: Basic Checks

1.  **Ensure Bluetooth is Enabled:** Check the Bluetooth icon in the top-right system tray or go to `Settings` > `Bluetooth` and make sure it is toggled on.
...
Ensure you have the core Bluetooth utilities installed. `blueman` is a more advanced Bluetooth manager that can sometimes solve issues the default GNOME manager cannot.
...
If problems persist, search online using your device's model name (e.g., "Sony WH-1000XM4 Ubuntu connection issues") to find solutions tailored to your hardware.
