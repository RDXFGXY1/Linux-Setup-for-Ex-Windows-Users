# Troubleshooting USB Drive Mounting on Fedora
...
Normally, when you plug in a USB drive, Fedora's desktop environment automatically "mounts" it and creates a shortcut in the Files app sidebar. You simply click it to access your files. The issues below are for when this process fails.
...
If the hardware is detected but not mounting, it's often a filesystem problem. Fedora may not have all filesystem tools installed by default, especially for proprietary ones from Microsoft.

1.  **Install Filesystem Tools:** For full read/write support for NTFS and exFAT, you may need to install packages from the main Fedora repos and the RPM Fusion repository.
    ```bash
    # Ensure RPM Fusion is enabled first
    # For exFAT
